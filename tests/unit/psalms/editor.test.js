...
test('psalm editor build: textarea is hidden after Build (melody view shown)', () => {
    ...
    assert.ok(
        ...
        'textarea should be hidden after Build (melody editor shown)'
    );
});
...
test('psalm editor build: verse rows are labeled "Verse N"', () => {
    ...
    assert.ok(
        labels.some((l) => l.textContent === 'Verse 1'),
        'should have "Verse 1" label'
    );
    assert.ok(
        labels.some((l) => l.textContent === 'Verse 2'),
        'should have "Verse 2" label'
    );
    assert.ok(
        !labels.some((l) => l.textContent.startsWith('Stanza')),
        'should not use "Stanza" label'
    );
});
...
test('psalm editor tone: termination options repopulated on tone change', () => {
    ...
    const _termCountBefore = termSelect.options.length;
    ...
    assert.ok(
        termSelect.options.length >= 1,
        'termination select should have options after tone change'
    );
});
...
test('psalm editor solemn: toggling solemn re-points note inputs', () => {
    ...
    const _notesBefore = input.value;
...
test('psalm editor Fit Text: re-points notes when text changes', () => {
    ...
    const notesAfter = document.querySelector('.editor-melody-input').value;
    assert.ok(
        notesAfter.trim().length > 0,
        'Fit Text must produce non-empty notes for the new verse'
    );
    assert.notEqual(
        notesAfter,
        notesBefore,
        'Fit Text with different text should re-point and produce different notes'
    );
});
...
test('psalm editor Fit Text: new syllables reflected in track chips', () => {
    ...
    assert.ok(
        chips.some((c) => c.textContent.includes('Pa') || c.textContent.includes('Bože')),
...
test('psalm editor Fit Text: | markers in textarea are stable (no duplication)', () => {
    ...
    assert.equal(
        rawAfterFit,
        rawAfterBuild,
        'Fit Text on already-marked text should not duplicate | markers'
    );
});
...
test('psalm selector: textarea is populated and Build runs after psalm select', async () => {
    ...
    assert.ok(
        ...
        'Edit melody toggle should appear after psalm load'
    );
});
...
test('psalm editor repeat intonation: checking on long verse adds intonation role to verse 2 AST', () => {
    ...
    assert.ok(
        !state.stanzas[1].ast.some((t) => t.role === 'intonation'),
...
test('psalm editor AST sync: placing the cadence accent note moves the acc role to that syllable', () => {
    ...
    const sylTokens = state.stanzas[1].ast
        .slice(0, mediantEnd)
