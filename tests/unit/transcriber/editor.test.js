...
import { initEditor, rebuildStanzas } from '../../../js/transcriber/editor.js';
...
function fire(el, type) {
    el.dispatchEvent(
        new document.defaultView[
            type === 'keyup' ? 'KeyboardEvent' : type === 'blur' ? 'FocusEvent' : 'Event'
        ](type, { bubbles: true })
    );
}
...
test('editor Re-Build: resets melody to rectotone defaults', () => {
    ...
    assert.ok(
        state.stanzas[0].lines[0].notes.startsWith('f f f'),
