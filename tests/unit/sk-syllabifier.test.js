...
test('syllabifyWord: miesto (ie diphthong)', () =>
    assert.deepEqual(split('miesto'), ['mies', 'to']));
test('syllabifyWord: dieťa (ie diphthong)', () => assert.deepEqual(split('dieťa'), ['die', 'ťa']));
test('syllabifyWord: viac (ia diphthong, single syllable)', () =>
    assert.deepEqual(split('viac'), ['viac']));
test('syllabifyWord: bielych (ie diphthong)', () =>
    assert.deepEqual(split('bielych'), ['bie', 'lych']));
...
test('syllabifyWord: hasNucleus true for vowel word', () =>
    assert.equal(sk.syllabifyWord('tebou').hasNucleus, true));
test('syllabifyWord: hasNucleus true for syllabic consonant (vlk)', () =>
    assert.equal(sk.syllabifyWord('vlk').hasNucleus, true));
test('syllabifyWord: hasNucleus false for consonant-only preposition (v)', () =>
    assert.equal(sk.syllabifyWord('v').hasNucleus, false));
test('syllabifyWord: hasNucleus false for consonant-only preposition (s)', () =>
    assert.equal(sk.syllabifyWord('s').hasNucleus, false));
...
test('syllabify: o tebe (vowel word, stands alone)', () =>
    assert.deepEqual(syls('o tebe'), ['o', 'te', 'be']));

test('syllabifyWord: machať (ch digraph fixed)', () =>
    assert.deepEqual(split('machať'), ['ma', 'chať']));
test('syllabifyWord: medza (dz digraph fixed)', () =>
    assert.deepEqual(split('medza'), ['me', 'dza']));
test('syllabifyWord: ucho (ch digraph between vowels)', () =>
    assert.deepEqual(split('ucho'), ['u', 'cho']));
...
test('syllabifyWord: sestra (str cluster fixed)', () =>
    assert.deepEqual(split('sestra'), ['ses', 'tra']));
test('syllabifyWord: demokracia', () =>
    assert.deepEqual(split('demokracia'), ['de', 'mok', 'ra', 'cia']));
test('syllabifyWord: vypracovať', () =>
    assert.deepEqual(split('vypracovať'), ['vyp', 'ra', 'co', 'vať']));
test('syllabifyWord: rozum', () => assert.deepEqual(split('rozum'), ['ro', 'zum']));
test('syllabifyWord: mestský (4-consonant cluster fixed)', () =>
    assert.deepEqual(split('mestský'), ['mes', 'tský']));
test('syllabifyWord: zmŕtvychvstanie (ch digraph + cluster fixed)', () =>
    assert.deepEqual(split('zmŕtvychvstanie'), ['zmŕt', 'vych', 'vsta', 'nie']));
test('syllabifyWord: najneobhospodarovávateľnejšieho', () =>
    assert.deepEqual(split('najneobhospodarovávateľnejšieho'), [
        'naj',
        'ne',
        'ob',
        'hos',
...
test('control char |: dnes|ka produces two chips with the same wordIdx (no GABC space)', () => {
    ...
    assert.deepEqual(
        tokens.map((t) => ({ syl: t.syl, wordIdx: t.wordIdx })),
        [
            { syl: 'dnes', wordIdx: 0 },
            { syl: 'ka', wordIdx: 0 },
        ]
...
test('control char |: Slo|ven|sku produces three chips all with the same wordIdx', () => {
    ...
    assert.ok(
        tokens.every((t) => t.wordIdx === tokens[0].wordIdx),
        'all share wordIdx'
    );
    assert.deepEqual(
        tokens.map((t) => t.syl),
...
test("isStressed: 'naozaj' (starts with 'na') remains stressed on first syllable", () => {
    ...
    assert.equal(
        tokens[0].isStressed,
        true,
        'first syllable of polysyllabic word is still stressed'
    );
});
