...
test('parseText: coda extracted from last word', () => {
    ...
    assert.ok(
        !lastLineSyls.includes('A') && !lastLineSyls.includes('men'),
...
test('Fit Text: single-stanza GABC into multi-stanza text — only stanza 0 preserves melody', () => {
    ...
    assert.equal(state.stanzas[0].lines[0].notes, importedNotes, 'stanza 0 melody preserved');
    assert.ok(state.stanzas[1].lines[0].notes.startsWith('f'), 'stanza 1 gets rectotone default');
});
