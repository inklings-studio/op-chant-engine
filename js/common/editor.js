...
export function renderTrack(trackEl, syllables, parsedNotes, wordMap) {
    ...
    function _flushGroup() {
        if (curGroupEl) {
            trackEl.appendChild(curGroupEl);
            curGroupEl = null;
            curWordIdx = null;
        }
    }
...
export function highlightCursorChip(input, track) {
    ...
    track
        .querySelectorAll('.track-chip-matched, .track-chip-empty, .track-chip-cursor')
        .forEach((chip, i) => {
            const active = i === idx;
...
export function clearCursorHighlight(track) {
    track.querySelectorAll('.track-chip-cursor').forEach((chip) => {
        ...
        chip.classList.add(
            chip.dataset.isMatched === '1' ? 'track-chip-matched' : 'track-chip-empty'
        );
    });
}
...
export function createMelodyInputRow(line, si, li, { onUpdate }) {
    ...
    input.type = 'text';
    input.className =
        'editor-melody-input w-full font-mono text-xs border border-op-tan rounded px-2 py-1 mb-1 focus:outline-none focus:ring-1 focus:ring-op-gold';
    ...
    input.addEventListener('input', () => {
        ...
        renderTrack(
            track,
            updatedLine.syllables,
            updatedLine.parsedNotes,
            updatedLine.wordMap ?? null
        );
