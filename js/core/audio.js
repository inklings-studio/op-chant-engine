...
function withAudioContext(fn) {
    ...
    if (ctx && ctx.state === 'suspended') {
        ctx.resume()
            .then(fn)
            .catch((err) => console.error('[audio.js] context.resume failed:', err));
    } else {
        fn();
    }
}
...
export function getScoreNotes(score) {
    return [].concat(...score.notations.map((n) => n.notes || [n])).filter((n) => !n.isAccidental);
}
...
function clearHighlight() {
    if (_currentNoteEl) {
        _currentNoteEl.classList.remove('active');
        _currentNoteEl = null;
    }
    if (_currentSyllableEl) {
        _currentSyllableEl.classList.remove('active');
        _currentSyllableEl = null;
    }
}
...
function ensureDefaultStartPitch(score, notes) {
    ...
    score.defaultStartPitch = new exsurge.Pitch(
        startPitch + (EXSURGE_G4 - Math.floor((low + high) / 2))
    );
}
            ...
            _timers.push(
                setTimeout(() => {
                    ...
                    if (note.pitch && !note.isDivider) {
                        ...
                        if (freq)
                            window.tones.playFrequency(freq, { length: duration * beatMs * 0.88 });
                    }
                }, delay)
            );
